...
##OS.append(Val)
##OS.append(Sal)
# LTM - don't add smart kids yet because there may be more than 20 and that throws
# off the indexing for normal kids since everything is based on SeatsPerRow.
#for k in SKids:
#    OS.append(k)
for k in kids:
    OS.append(k)
...
def process(OS, row_offset, SeatsPerRow, SeatsPerFullRow):
    last_index = 0;
    for idx, name in enumerate(OS):
          
        sec = (int(idx/SeatsPerRow) + 1)%2;
        row = int(idx/SeatsPerFullRow) + 1 + row_offset;
        seat = (idx)%SeatsPerRow + 1;
        if ((SeatsPerFullRow % 2) and not ((idx+1) % SeatsPerFullRow)):
            sec = 2
            seat = SeatsPerRow + 1
...
process(SKids, 0, len(SKids)/2, len(SKids))
process(OS, 1, SeatsPerRow, SeatsPerFullRow)
sA.close()
sB.close()
